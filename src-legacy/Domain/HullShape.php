...
final class HullShape
{
     ...
     */
    private function __construct(array $vertices, array $faceAxes, array $edgeDirections, string $volume, int $maxCoordinate, int $maxAxisComponent)
    {
        $this->vertices = $vertices;
        $this->faceAxes = $faceAxes;
        $this->edgeDirections = $edgeDirections;
        $this->volume = $volume;
        $this->maxCoordinate = $maxCoordinate;
        $this->maxAxisComponent = $maxAxisComponent;
    }
     ...
     * @param list<array{int,int,int}> $vertices
     */
    public static function shapeFor(array $vertices, string $rotation): self
    {
        $key = $rotation . '|' . implode(';', array_map(
            static function (array $vertex): string {
     ...
     */
    public static function rotate(array $vertices, string $rotation): array
    {
        switch ($rotation) {
            case Rotation::LWH:
                $axes = [0, 1, 2];
    ...
    private static function windFace(array $face, array $outward): array
    {
        ...
        usort($face, static function (array $l, array $r): int {
    ...
    private static function orderedAxes(array $axes): array
    {
        ...
        usort($ordered, static function (array $l, array $r): int {
            return [$l[0], $l[1], $l[2]] <=> [$r[0], $r[1], $r[2]];
        });
        return $ordered;
    }
