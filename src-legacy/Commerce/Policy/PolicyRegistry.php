...
final class PolicyRegistry
{
    ...
    {
        $ids = array_map(static function (array $pin): string {
            return $pin[0];
        }, $pins);
        ...
        return array_map(function (array $pin): Rule {
            return $this->version($pin[0], $pin[1]);
        }, $pins);
    }
     ...
     * @param mixed $scope
     */
    public function evaluate($scope, array $context, int $asOf): Decision
    {
        $matching = [];
     ...
     * @param mixed $scope
     */
    public static function decide(array $rules, $scope, array $context): Decision
    {
        $rejects = [];
        $allows = [];
