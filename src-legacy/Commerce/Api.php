...
final class Api
{
    ...
    public static function quote(array $document, array $request): array
    {
        ...
        $rating = Shape::model('request', static function () use ($request, $accessorials) {
    ...
    {
        $present = array_values(array_filter($names, static function (string $name) use ($request): bool {
            return ($request[$name] ?? null) !== null;
        }));
        if (count($present) !== 1) {
     ...
     * @param mixed $scope
     */
    private static function decide(PolicyRegistry $policies, $scope, array $context, array $request, string $pin): Decision
    {
        if ($pin === 'as_of') {
    ...
    /** @return list<Rule>
     * @param mixed $value */
    private static function pinnedRules(PolicyRegistry $policies, $value): array
    {
        $pins = [];
    ...
    private static function resolveCatalog(array $catalogs, string $catalogId, ?int $version, ?int $asOf): Version
    {
        ...
        } catch (CatalogVersionNotFoundException $exception) {
            throw new Rejection('catalog_version_not_found', $selector);
        } catch (NoEffectiveVersionException $exception) {
            throw new Rejection('no_effective_catalog_version', $selector);
        } catch (AmbiguousReferenceException $exception) {
    ...
    private static function sortKeys($value)
    {
        ...
        if ($arrayIsListFunction($value)) { return array_map(\Closure::fromCallable([self::class, 'sortKeys']), $value); }
        ksort($value, SORT_STRING);
        return array_map(\Closure::fromCallable([self::class, 'sortKeys']), $value);
    }
}
