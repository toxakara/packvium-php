...
final class SolverOrchestrator
{
    ...
    public function __construct(array $customConstraints=[], array $customOrders=[], array $customSolvers=[], ?CandidateScorer $candidateScorer=null, ?ContainerSelector $containerSelector=null)
    {
        $this->customConstraints = $customConstraints;
        $this->customOrders = $customOrders;
        $this->customSolvers = $customSolvers;
    ...
    private function solveSequential(array $items,array $containers,PackingConfig $config,Deadline $deadline,array $starts,?EffortBudget $effortBudget):PortfolioRun
    {
        $results=[];
        $records=array_map(
            static function (array $start): StartRecord {
                return new StartRecord($start[0]->name().':'.$start[1],false,false,false);
            },
            $starts,
        );
        foreach($starts as $position=>[$solver,$orderName,$order]){
            ...
            $startConfig=substr_compare($orderName, ':greedy', -strlen(':greedy')) === 0?self::greedyConfig($config):$config;
            ...
            if($effortReached){
                $unpacked=array_map(
                    static function (UnpackedItem $item): UnpackedItem {
                        return $item->reason==='time_limit'
    ...
    private function solveConcurrent(array $items,array $containers,PackingConfig $config,Deadline $deadline,array $starts,?EffortBudget $effortBudget):PortfolioRun
    {
        ...
        $results=array_values(array_filter($results,static function ($result): bool {
            return $result!==null;
        }));
        return $this->finishPortfolio($items,$containers,$deadline,$results,$records);
    }
    ...
    private function finishPortfolio(array $items,array $containers,Deadline $deadline,array $results,array $records):PortfolioRun
    {
        ...
        $records=array_map(
            static function (StartRecord $record) use ($globalDeadline): StartRecord {
                return $record->withGlobalDeadline($globalDeadline);
            },
            $records,
        );
        if($results===[]){
            ...
            $results[] = new RawSolution($fallbackId,[],array_map(
                function ($i) use ($containers) {
                    return new UnpackedItem($i,...$this->unpackedReason($i,$containers,true));
                },
                $items,
    ...
    private function supportIsTheBlocker(ItemInstance $item,array $packed,PackingConfig $config):bool
    {
        ...
        $withoutSupport=array_values(array_filter($constraints,static function ($c) {
            return !($c instanceof SupportConstraint);
        }));
        ...
        $budget=Deadline::ofMilliseconds(2000);
    ...
    private function starts(array $items,PackingConfig $config):array
    {
        ...
        $greedy=array_map(static function (array $start): array {
            return $start[0]->name()==='homogeneous_blocks'?$start:[$start[0],$start[1].':greedy',$start[2]];
        },$starts);
        ...
        return array_merge($greedy, $refinements);
    }

    private static function greedyConfig(PackingConfig $config):PackingConfig
    {
        return new PackingConfig(
            $config->profile,$config->timeLimitMs,$config->topK,$config->seed,$config->maxContainers,
            $config->clearance,$config->minimumSupportRatio,$config->exactItemLimit,$config->multiStartOrders,
            $config->validateResult,1,$config->maxCandidatePoints,$config->solvers,$config->objective,
            $config->effortBudget,$config->dimensionalWeightDivisor,$config->dimensionalWeightLengthUnit,
            $config->dimensionalWeightWeightUnit,$config->requirePlacementCoordinates,1,
            1,1,
        );
    }
    ...
    private function solvers(array $items,PackingConfig $config):array
    {
        ...
        if($config->solvers!==[]){
            ...
            return array_merge($this->customSolvers, $base);
        }
        $grid=new GridSolver();
    ...
    private function orders(array $items,PackingConfig $config):array
    {
        ...
        if($config->profile===SolverProfile::Quality)$strategies=[
            ['small_edge',static function (ItemInstance $i) use ($config): array {
                return array_merge(ItemOrdering::lead($i,$config), [$i->dimensions()->maxEdge()], $i->dimensions()->volumeKey(), [$i->id()]);
            }],
            ['small_volume',static function (ItemInstance $i) use ($config): array {
                return array_merge(ItemOrdering::lead($i,$config), $i->dimensions()->volumeKey(), [$i->dimensions()->maxEdge(), $i->id()]);
            }],
        ];
        $strategies=array_merge($strategies, [
            ['volume',static function (ItemInstance $i) use ($config): array {
                return array_merge(ItemOrdering::lead($i,$config), $i->dimensions()->descendingVolumeKey(), [-$i->dimensions()->maxEdge(), -$i->weight()->ticks, $i->id()]);
            }],
            ['base_area',static function (ItemInstance $i) use ($config): array {
                return array_merge(ItemOrdering::lead($i,$config), [-$i->dimensions()->baseAreaTicks()], $i->dimensions()->descendingVolumeKey(), [$i->id()]);
            }],
            ['weight',static function (ItemInstance $i) use ($config): array {
                return array_merge(ItemOrdering::lead($i,$config), [-$i->weight()->ticks], $i->dimensions()->descendingVolumeKey(), [$i->id()]);
            }],
            ['constrained',static function (ItemInstance $i) use ($config): array {
                return array_merge(ItemOrdering::lead($i,$config), [count($i->item->allowedRotations), -$i->item->minimumSupportRatio], $i->dimensions()->descendingVolumeKey(), [$i->id()]);
            }],
        ]);
        ...
        foreach($strategies as [$name,$key]){
            ...
            $sig=implode('|',array_map(static function ($i) {
                return $i->id();
            },$order));
    ...
    private function acrossContainers(SingleContainerSolver $solver,array $items,array $containers,PackingConfig $config,SearchStats $stats,Deadline $deadline):array
    {
        ...
        $maximum=$config->maxContainers??array_sum(array_map(static function ($c) use ($items) {
            return $c->quantity??count($items);
        },$containers));
        $ordered=StableSorter::sortBy($containers,static function (Container $c): array {
            return array_merge([$c->costMinor], $c->innerDimensions->volumeKey(), [$c->id]);
        });
        ...
        while($remaining!==[]&&count($packed)<$maximum){
            ...
            foreach($ordered as $container){
                ...
                catch(TimeLimitReached $exception){$reached=true;break;}
                ...
                if(($config->objective==='shipping_cost'||$config->objective==='lowest_landed_cost')&&$config->dimensionalWeightDivisor!==null){
                    ...
                    if($config->objective==='shipping_cost'){
                        ...
                        $charge=($nullsafeVariable1 = $container->rateTable) ? $nullsafeVariable1->chargeMinorOrNull(LandedCostSolutionScorer::grams($billed)) : null;
                        $candidateScore=[
            ...
            if($candidates===[])break;
            $candidates=StableSorter::sortBy($candidates,static function (array $candidate): array {
                return $candidate[0];
            });
            ...
            $remaining=array_values(array_filter($remaining,static function ($i) use ($ids) {
                return !isset($ids[$i->id()]);
            }));
            if($reached)break;
        }
        $unpacked=[];
    ...
    {
        $unpacked=array_map(static function (ItemInstance $item): UnpackedItem {
            return new UnpackedItem($item,'search_exhausted');
        },$remaining);
    ...
    {
        $available=array_values(array_filter($containers,static function (Container $container) use ($plan): bool {
            return $plan['inventory'][$container->id]===null||$plan['inventory'][$container->id]>0;
        }));
        if($plan['remaining']===[]||$available===[])return 0;
        $lower=0;
        if(!array_filter($plan['remaining'],static function (ItemInstance $item): bool {
        ...
        if(array_filter($available,static function (Container $container): bool {
            return $container->maxPayload===null;
        })===[]){
            $capacity=max(array_map(static function (Container $container): int {
                return $container->maxPayload->ticks;
            },$available));
            if($capacity>0){$required=array_sum(array_map(static function (ItemInstance $item): int {
                return $item->weight()->ticks;
            },$plan['remaining']));$lower=max($lower,intdiv($required+$capacity-1,$capacity));}
        }
        return $lower;
    }
    ...
    private static function planBound(array $plan,array $containers,PackingConfig $config):array
    {
        ...
        $score[]=implode('|',array_map(static function (ItemInstance $item): string {
            return $item->id();
        },$plan['remaining']));
        return $score;
    }
    ...
    private function acrossContainerPlans(SingleContainerSolver $solver,array $items,array $containers,PackingConfig $config,SearchStats $stats,Deadline $deadline):array
    {
        ...
        $maximum=$config->maxContainers??array_sum(array_map(static function (Container $container) use ($items): int {
            return $container->quantity??count($items);
        },$containers));
        $ordered=StableSorter::sortBy($containers,static function (Container $container): array {
            return array_merge([$container->costMinor], $container->innerDimensions->volumeKey(), [$container->id]);
        });
        while($beam!==[]&&$nodes<$config->containerPlanNodeLimit){
            ...
            foreach($beam as $plan){
                ...
                foreach($ordered as $container){
                    ...
                    $nodes++;
                    try{$one=$solver->packOne($container,$plan['sequence']+1,$plan['remaining'],$config,$stats,$deadline);}catch(TimeLimitReached $exception){$reached=true;break;}
                    $reached=$reached||$one->timeLimitReached;if($one->state->placementCount()===0)continue;
                    [$committed,$ids]=self::commitState($one->state);$child=$plan;$child['packed'][]=$committed;$child['remaining']=array_values(array_filter($plan['remaining'],static function (ItemInstance $item) use ($ids): bool {
                        return !isset($ids[$item->id()]);
                    }));$child['sequence']++;
            ...
            $dominant=[];
            foreach($expansions as $plan){$signature=implode('|',array_map(static function (ItemInstance $item): string {
                return $item->id();
            },$plan['remaining'])).'::'.json_encode($plan['inventory'],JSON_THROW_ON_ERROR);$previous=$dominant[$signature]??null;if($previous===null||self::planScore($plan,[],$config)<self::planScore($previous,[],$config))$dominant[$signature]=$plan;}
            ...
            $beam=array_slice(
                StableSorter::sortBy(array_values($dominant),static function (array $plan) use ($containers, $config): array {
                    return self::planBound($plan,$containers,$config);
                }),
