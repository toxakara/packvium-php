...
final class HomogeneousBlockSolver implements SingleContainerSolver
{
    ...
    private function solutionKey(SingleContainerSolution $solution):array
    {
        ...
        return array_merge([count($solution->unpacked)], array_map(static function (int $chunk): int {
            return -$chunk;
        },BigInt::chunks($solution->state->usedVolume)), [$solution->state->maxZ, implode('|',$signature)]);
    }
                    ...
                    foreach($prototype->dimensions()->uniqueRotations($prototype->item->allowedRotations) as [$rotation,$physical]){
                        ...
                        for($nx=1;$nx<=min($maximumX,$capacity);$nx++){
                            ...
                            for($ny=1;$ny<=$maximumNy;$ny++){
                                ...
                                $spaceFill=(int)BigInt::divide(BigInt::multiply($usedVolume,1000000),$spaceVolume);
                                $volumeKey=array_map(static function (int $chunk): int {
                                    return -$chunk;
                                },BigInt::chunks($usedVolume));
                                $lead=$mode==='count'?array_merge([-$count, -$spaceFill], $volumeKey):array_merge($volumeKey, [-$spaceFill, -$count]);
                                $score=array_merge($lead, [$space->origin->z, $space->origin->y, $space->origin->x, $itemId, $rotation, $nx, $ny, $nz]);
