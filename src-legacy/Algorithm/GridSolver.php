...
final class GridSolver implements SingleContainerSolver
{
    ...
    private static function latticeProfile(\Packvium\Domain\Item $item):string
    {
        ...
        return json_encode([
            ...
            ($nullsafeVariable1 = $item->maxTopLoad) ? $nullsafeVariable1->ticks : null,$tags,$incompatibleTags,$eligibleContainerTags,
            ...
            ($nullsafeVariable2 = $item->nestingHeight) ? $nullsafeVariable2->ticks : null,$item->group,
    ...
    {
        return $items!==[]&&count(array_unique(array_map(static function ($i) {
            return self::latticeProfile($i->item);
        },$items)))===1;
    }
    ...
    public function packOne(Container $container,int $sequence,array $items,PackingConfig $config,SearchStats $stats,Deadline $deadline):SingleContainerSolution
    {
        ...
        $nestingTicks=(($nullsafeVariable3 = $prototype->nestingHeight) ? $nullsafeVariable3->ticks : null)??0;
        $best=null;
        foreach($prototype->dimensions->uniqueRotations($prototype->allowedRotations) as [$rotation,$physical]){
            ...
            $score=array_merge([-$capacity],$envelope->volumeKey(),[$rotation]);
        ...
        if($prototype->group!==null&&$total<count($items))return new SingleContainerSolution($state, $items, false, false, true);
        $clearance=$config->clearance->ticks;
        $sameDeclaredType=count(array_unique(array_map(static function ($i) {
            return $i->item->id;
        },array_slice($items,0,$total))))===1;
