...
final class Packer
{
    /**
     * @readonly
     * @var \Packvium\Config\PackingConfig
     */
    private $config;
    /**
     * @readonly
     * @var \Packvium\Extension\ExtensionRegistry
     */
    private $extensions;
    /**
     * @readonly
     * @var \Packvium\Objective\SolutionScorer|null
     */
    private $solutionScorer;
    /**
     * @readonly
     * @var \Closure|null
     */
    private $clock;
    public function __construct(?PackingConfig $config = null, ?ExtensionRegistry $extensions = null, ?SolutionScorer $solutionScorer = null, ?\Closure $clock = null)
    {
        $config = $config ?? new PackingConfig();
        $extensions = $extensions ?? new ExtensionRegistry();
        $this->config = $config;
    ...
    public function pack(array $items, array $containers): PackingResult
    {
        ...
        foreach ($portfolio->solutions as $raw) {
            ...
            $ranked[] = new PackingResult($status, $raw->containers, $raw->unpacked, new AlgorithmReport(
                $this->config->profile,
                $raw->solverName,
                $deadline->elapsedMs(),
                $this->config->seed,
                ...
                $raw->stats->metrics(),
            ), $score, $warnings, [], null, $termination, null, $this->config->objective);
        }

        $ranked = StableSorter::sortBy($ranked, static function ($result): array {
            return [
        ...
        $valid = array_values(array_filter(
            ...
            static function ($result): bool {
                return $result->status !== PackingStatus::InvalidResult;
            },
        ));
        ...
        $alternatives = array_slice(
            array_values(array_filter(
                ...
                function (PackingResult $result): bool {
                    return LandedCostSolutionScorer::unpriceableContainer($result->containers, $this->config) === null;
                },
            )),
            0,
