...
final class RationalParser
{
     ...
     */
    public static function parse(int|string $value): array
    {
    ...
    private static function round(int $n, int $d, Rounding $rounding): int
    {
        $sign = $n < 0 ? -1 : 1; $a = abs($n); $q = intdiv($a, $d); $r = $a % $d;
        if ($r === 0) { return $sign * $q; }
        return match($rounding) {
            Rounding::Floor => $sign < 0 ? -($q + 1) : $q,
            Rounding::Ceil => $sign < 0 ? -$q : $q + 1,
            Rounding::Nearest => $sign * (($r * 2 < $d) ? $q : (($r * 2 > $d) ? $q + 1 : ($q % 2 === 0 ? $q : $q + 1))),
        };
    }
