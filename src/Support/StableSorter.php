...
final class StableSorter
{
    ...
    public static function sort(array $values, callable $compare): array
    {
        ...
        usort($decorated, static fn(array $a,array $b):int => $compare($a['v'],$b['v']) ?: ($a['i'] <=> $b['i']));
        return array_values(array_map(static fn(array $x)=>$x['v'],$decorated));
    }
    ...
    public static function sortBy(array $values, callable $key): array
    {
        ...
        usort($decorated, static fn(array $a,array $b):int => ($a['k'] <=> $b['k']) ?: ($a['i'] <=> $b['i']));
        return array_values(array_map(static fn(array $x)=>$x['v'],$decorated));
    }
}
