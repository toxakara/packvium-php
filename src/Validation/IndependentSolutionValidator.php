...
final class IndependentSolutionValidator
{
    ...
    public function validate(PackingRequest $request,array $containers,float $minimumSupportRatio=0.0,?Length $clearance=null,?array $unpacked=null):ValidationReport
    {
        ...
        $clearanceTicks=$clearance?->ticks??0;
    ...
    private static function collisionPairs(array $placements):array
    {
        ...
        foreach($buckets as $indices){
            ...
            usort($ordered,static fn(array $a,array $b):int=>[$a[0],$a[1],$a[2]]<=>[$b[0],$b[1],$b[2]]);
            $active=[];
    ...
    {
        $placements=$packed->placements;
        $boxes=array_map(static fn(Placement $p)=>$p->box(),$placements);
        $stops=array_map(static fn(Placement $p)=>$p->instance->item->stopIndex,$placements);
        try{
            ...
            $stuckIds=array_map(static fn(int $i)=>$placements[$i]->instance->id(),$error->stuck);
            sort($stuckIds);
