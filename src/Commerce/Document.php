...
final readonly class Document
{
    ...
    private function __construct(
        public CarrierRegistry $carriers,
        public PolicyRegistry $policies,
    ...
    private static function loadTariffs(mixed $value): CarrierRegistry
    {
    ...
    private static function publishTariffVersions(CarrierRegistry $registry, string $carrierId, string $serviceId, mixed $value, string $parent): void
    {
        ...
        foreach ($versions as $index => $entry) {
            ...
            Shape::model($path, static fn() => $registry->publish(
                $carrierId, $serviceId,
    ...
    private static function loadAccessorials(mixed $value, string $path): array
    {
        $charges = [];
        foreach (Shape::listOf($value, $path) as $index => $entry) {
            ...
            $charges[$id] = Shape::model($entryPath, static fn() => new AccessorialCharge(
                $id,
    ...
    private static function loadPolicyRules(mixed $value): PolicyRegistry
    {
    ...
    private static function publishRuleVersions(PolicyRegistry $registry, string $ruleId, mixed $value, string $parent): void
    {
        $versions = Shape::listOf($value, "{$parent}.versions");
    ...
    private static function loadPredicates(mixed $value, string $path): array
    {
        $predicates = [];
        foreach (Shape::listOf($value, $path) as $index => $entry) {
            $entryPath = "{$path}[{$index}]";
            ...
            $predicates[] = Shape::model($entryPath, static fn() => new Predicate(
    ...
    private static function loadCatalogs(mixed $value): array
    {
        $catalogs = [];
        foreach (Shape::listOf($value, 'document.catalogs') as $index => $entry) {
            ...
            $registry = Shape::model($path, static fn() => new CatalogRegistry($catalogId));
    ...
    private static function publishCatalogVersions(CatalogRegistry $registry, mixed $value, string $parent): void
    {
        $versions = Shape::listOf($value, "{$parent}.versions");
        ...
        foreach ($versions as $index => $entry) {
            ...
            if (array_key_exists('rollback_to', $fields)) {
                ...
                Shape::model($path, static fn() => $registry->rollback(
    ...
    private static function loadSnapshot(mixed $value, string $path): Snapshot
    {
        ...
        return Shape::model($path, static fn() => new Snapshot(
            $collect('items', self::loadItem(...)),
            $collect('cartons', self::loadCarton(...)),
            $collect('pallets', self::loadPallet(...)),
            $collect('exclusions', self::loadExclusion(...)),
            $collect('overrides', self::loadOverride(...)),
        ));
    }
    ...
    private static function loadItem(array $fields, string $path): ItemMaster
    {
        ...
        return Shape::model($path, static fn() => new ItemMaster(
    ...
    private static function loadCarton(array $fields, string $path): CartonMaster
    {
        ...
        return Shape::model($path, static fn() => new CartonMaster(
            Shape::text($fields['id'], "{$path}.id"),
    ...
    private static function loadPallet(array $fields, string $path): PalletMaster
    {
        ...
        return Shape::model($path, static fn() => new PalletMaster(
            Shape::text($fields['id'], "{$path}.id"),
    ...
    private static function loadExclusion(array $fields, string $path): ExclusionRule
    {
        ...
        return Shape::model($path, static fn() => new ExclusionRule(
            Shape::text($fields['id'], "{$path}.id"),
    ...
    private static function loadOverride(array $fields, string $path): FacilityOverride
    {
        ...
        $override = match ($kind) {
            'item' => self::loadItem($payload, "{$path}.override"),
            'carton' => self::loadCarton($payload, "{$path}.override"),
            'pallet' => self::loadPallet($payload, "{$path}.override"),
            default => Shape::fail("{$path}.kind", "expected one of ['carton', 'item', 'pallet']"),
        };
        return Shape::model($path, static fn() => new FacilityOverride(
            Shape::text($fields['id'], "{$path}.id"),
