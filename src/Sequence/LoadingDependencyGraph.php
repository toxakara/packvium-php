...
final class LoadingDependencyGraph
{
    ...
    public static function build(array $boxes): self
    {
        ...
        foreach ($boxes as $index => $box) {
            $supporterIndexes = array_map(static fn($edge) => $edge->index, $contacts->supporters($index));
    ...
    {
        $boxes = array_map(static fn(Placement $placement): AxisAlignedBox => $placement->envelopeBox(), $placements);
    ...
    public static function verifyLoadingPrefixBusinessRules(array $placements, array $order, Container $container): void
    {
        ...
        $maxDensityTicks = $container->maxStackDensity?->ticks;
        $present = [];
        foreach ($order as $step => $index) {
