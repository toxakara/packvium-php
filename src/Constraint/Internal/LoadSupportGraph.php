...
final class LoadSupportGraph
{
    ...
    private ContactGraph $face;
    /** @var list<LoadUnit> */
    private array $units;
    private bool $nested;
    /** @var list<int>|null */
    private ?array $descending=null;
    /** @var list<int>|null */
    private ?array $priorDescending=null;
    ...
    public function __construct(array $units, int $cellHint = 1)
    {
        ...
        $face=new ContactGraph(array_map(static fn(LoadUnit $unit):AxisAlignedBox=>$unit->box,$units),$cellHint);
        $this->face=$face;
        $this->units=$units;
        $this->nested=$nesting!==[];
        if($nesting===[]){
