...
final class AxleLoad
{
    ...
    {
        if (str_starts_with($numerator, '-')) { return false; }
    ...
    {
        $aNeg = str_starts_with($a, '-'); $bNeg = str_starts_with($b, '-');
    ...
    {
        if (!str_starts_with($numerator, '-')) { return BigInt::divide($numerator, $denominator); }
